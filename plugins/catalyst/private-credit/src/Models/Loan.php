<?php

namespace Catalyst\PrivateCredit\Models;

use Illuminate\Database\Eloquent\Factories\HasFactory;
use Illuminate\Database\Eloquent\Model;
use Illuminate\Database\Eloquent\Relations\BelongsTo;
use Illuminate\Database\Eloquent\Relations\MorphMany;

class Loan extends Model
{
    use HasFactory;

    protected $table = 'private_credit_loans';

    protected $fillable = [
        'borrower_id',
        'loan_status',
        'principal_amount',
        'loan_release_date',
        'loan_duration',
        'duration_period',
        'interest_method',
        'interest_rate',
        'interest_cycle',
        'repayment_cycle',
        'custom_repayment_schedule_enabled',
        'late_penalty_is_active',
        'late_penalty_type',
        'late_penalty_calculate_on',
        'late_penalty_amount',
        'late_penalty_grace_period',
        'late_penalty_recurring',
        'funding_account_id',
        'loans_receivable_account_id',
        'default_interest_income_account_id',
        'default_fees_income_account_id',
        'default_penalty_income_account_id',
        'default_overpayment_account_id',
        'collateral_name',
        'collateral_description',
        'collateral_defects',
        'collateral_files',
    ];

    protected $casts = [
        'custom_repayment_days' => 'array',
        'loan_release_date' => 'date',
        'custom_repayment_schedule_enabled' => 'boolean',
        'late_penalty_is_active' => 'boolean',
        'collateral_files' => 'array',
    ];

    /**
     * Get all of the loan's fees.
     */
    public function fees(): MorphMany
    {
        return $this->morphMany(Fee::class, 'feesable');
    }

    public function borrower(): BelongsTo
    {
        return $this->belongsTo(\App\Models\User::class, 'borrower_id'); // Assuming App\Models\User is the borrower model
    }

    public function fundingAccount(): BelongsTo
    {
        return $this->belongsTo(FundingAccount::class);
    }

    public function loansReceivableAccount(): BelongsTo
    {
        return $this->belongsTo(ChartOfAccount::class, 'loans_receivable_account_id');
    }

    public function defaultInterestIncomeAccount(): BelongsTo
    {
        return $this->belongsTo(ChartOfAccount::class, 'default_interest_income_account_id');
    }

    public function defaultFeesIncomeAccount(): BelongsTo
    {
        return $this->belongsTo(ChartOfAccount::class, 'default_fees_income_account_id');
    }

    public function defaultPenaltyIncomeAccount(): BelongsTo
    {
        return $this->belongsTo(ChartOfAccount::class, 'default_penalty_income_account_id');
    }

    public function defaultOverpaymentAccount(): BelongsTo
    {
        return $this->belongsTo(ChartOfAccount::class, 'default_overpayment_account_id');
    }

    /**
     * Delete loan record and related fee records.
     */
    public static function destroy($ids)
    {
        $ids = value(function () use($ids) {
            if ($ids instanceof \Illuminate\Database\Eloquent\Collection) return $ids->modelKeys();
            if ($ids instanceof \Illuminate\Support\Collection) return $ids->all();
            return is_array($ids) ? $ids : func_get_args();
        });

        if (count($ids) === 0) return 0;

        $ids = array_map(
            function ($id) {
                self::fees()->where('id', $id)->destroy();
                if (self::where('id', $id)->delete()) return $id;
            }, $ids
        );

        return count($ids);
    }
}
